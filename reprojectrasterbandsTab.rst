...
.. figure:: _static/reproject_raster_bands.jpg
	:align: center
	:width: 500pt
	
	|clip_tool| :guilabel:`Reproject raster bands`
...
* |checkbox| :guilabel:`Align to raster` |checkbox| :guilabel:`same extent as reference` |input_list| |optional|: if :guilabel:`Align to raster` is checked, use the same coordinate reference system and pixel size as the selected reference; if :guilabel:`same extent as reference` is checked, the output extent will be the same as the reference;
* |reload|: refresh layer list;
* |checkbox| :guilabel:`Use EPSG code` |input_text| :guilabel:`X resolution` |input_text| :guilabel:`Y resolution` |input_text| |optional|: if checked, use the EPSG code for defining the output coordinate reference system with :guilabel:`X resolution` and :guilabel:`Y resolution`;
...
* |checkbox| :guilabel:`Change output NoData value` |input_number| |optional|: if checked, set a new value for ``NoData`` pixels;
...
* :guilabel:`RUN` |run|: choose an output destination and start the process;
